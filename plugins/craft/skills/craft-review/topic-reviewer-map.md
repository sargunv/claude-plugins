...
| `idioms`         | Language/framework conventions — emit as `idioms-<lang>` or `idioms-<framework>` (e.g. `idioms-go`, `idioms-react`). Emit one tag per distinct stack found in changed files. |         | `reviewer-idioms` (parameterized by stack)     |
...
- `idioms` is conditional — the tagger emits `idioms-typescript`, `idioms-go`, `idioms-python`, etc.
  based on file extensions in the diff. One tag per detected stack. The orchestrator spawns one
